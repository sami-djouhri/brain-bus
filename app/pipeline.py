...
def _render_hint(rule: Rule, trigger_event: Event | None = None) -> str:
    ...
    if not hint or "{{" not in hint:
        return hint
    # ★★ Gegen das AUSLOESENDE Ereignis rendern, nicht gegen das zuletzt
    # eingetroffene. Zwischen Ausloesen und Zustellen liegt der LLM-Aufruf; melden
    # mehrere Subjekte gleichzeitig, ist events[-1] dann ein anderes Subjekt und die
    # Meldung nennt den falschen Namen (Beleg an trigger_event in rules.py).
    # Das Ereignis wird als WERT durchgereicht, nicht ueber rule.trigger_event
    # gelesen: bei mehreren Feuerungen derselben Regel ueberschreibt die zweite das
    # Feld, bevor die erste Meldung gerendert ist. Genau der Tapo-Fall.
    # Rueckfall auf events[-1] nur, wenn kein Ereignis mitgegeben wurde.
    event = trigger_event or (rule.events[-1] if rule.events else None)
...
def _build_reasoning_prompt(rule: Rule, trigger_event: Event | None = None) -> tuple[str, str]:
    system = "Antwort auf Deutsch mit genau 2 sehr kurzen Saetzen. Nur Ursache und naechster Check."
    # Auch der Prompt bekommt das ausloesende Ereignis: sonst deutet das Modell ein
    # fremdes Subjekt und die "Einschaetzung (KI)" widerspricht der Messung darueber.
    event = trigger_event or rule.events[-1]
...
async def run_reasoning(
    ...
    `trigger_event` ist das Ereignis, das diese Meldung ausgeloest hat. Es wird
    mitgegeben statt hier aus rule.events gelesen: zwischen Ausloesen und diesem
    Aufruf liegt der LLM-Aufruf, und bei mehreren Subjekten zeigt events[-1] dann
    auf ein fremdes (siehe trigger_event in rules.py).
    """
    event = trigger_event or (rule.events[-1] if rule.events else None)
    ...
    if _priority_rank(_effective_ntfy(rule).get("priority", "")) <= _priority_rank("low"):
        ...
        system, user = _build_reasoning_prompt(rule, event)
        if diag_block:
    ...
    if irc is not None:
        hinweis = _render_hint(rule, event) or rule.id

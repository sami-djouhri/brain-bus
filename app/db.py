...
-- V1 „Gläserne Autonomie", 2. Schritt: Entscheidungen GEGEN Handeln.
-- Wenn decide.judge() eine Aktion erwägt und sich (mit Begründung) DAGEGEN
-- entscheidet (recommend_action=False oder Confidence < Schwelle), war das
...
def prune_decisions(before_iso: str) -> int:
    """Retention: hält die decisions-Tabelle beschränkt (billig, indexiert)."""
    with connect() as conn:
        cur = conn.execute("DELETE FROM decisions WHERE created_at < ?", (before_iso,))
        return cur.rowcount


def list_decisions(limit: int = 50, since_iso: str | None = None) -> list[dict[str, Any]]:
